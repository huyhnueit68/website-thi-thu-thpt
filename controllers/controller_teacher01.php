...
/**
 * Controller Teacher Test
 * Author: Phạm Quang Huy (Harry Pham)
 * Mail: pqhuy2@software.misa.com.vn
 **/

require_once('models/model_teacher01.php');
require_once('views/view_admin.php');
require_once('views/view_teacher01.php');
...
class Controller_Teacher01
{
    ...
    public function add_test($test_code, $test_name, $grade_id, $subject_id, $total_questions, $time_to_do, $note)
    {
        $model = new Model_Teacher01();
        return $model->add_test($test_code, $test_name, $grade_id, $subject_id, $total_questions, $time_to_do, $note);
    }
    ...
    public function get_dashboard_info()
    {
        ...
        $total = array($student, $grade, $class, $subject, $question, $test);
        return $total;
    }
    ...
    /**
     * Hàm xử lý lấy dữ liệu kiểm tra tạo đề thi
     * CreatedBy: PQ Huy (05.11.2021)
     */
    public function check_add_test()
    {
        ...
        if (empty($test_name) || empty($time_to_do)) {
            ...
            do {
                ...
                $add = $this->add_test($test_code, $test_name, $grade_id, $subject_id, $total_questions, $time_to_do, $note);
            } while (!$add);
            $result['status_value'] = "Thêm thành công!";
            $result['status'] = 1;
    ...
    public function export_score()
    {
        ...
        $sheet->setCellValue('A1', 'Danh Sách Điểm Đề thi ' . $test_code);
